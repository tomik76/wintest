...
int main() {
    ...
    for (int i = 0; i < 10000; i++) { 
        std::this_thread::sleep_for(std::chrono::seconds(5));
        TypeKeyboard("Long comment The error  is typically encountered when using the Cl.exe compiler to build a C++ project, and indicates that the linker is unable to find the C++ Standard Library (libstdc++.dll) that is required to build your project.");
